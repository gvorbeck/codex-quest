...
import { doc, query, where, getDocs, collection, documentId, onSnapshot } from "firebase/firestore";
...
export function useDataResolver(options: UseDataResolverOptions = {}) {
  const [resolvedData, setResolvedData] = useState<Map<string, ResolvedData>>(new Map());
  ...
  const getCacheKey = (userId: string, characterId: string) => `${userId}:${characterId}`;
  ...
  const fetchCharactersBatch = async (userId: string, characterIds: string[]): Promise<Map<string, { name: string; avatar?: string }>> => {
    ...
    try {
      ...
      const batchPromises = batches.map(async (batch) => {
        ...
        const batchResults = new Map<string, { name: string; avatar?: string }>();
      ...
      console.warn(`Failed to batch fetch character data for ${userId}:`, error);
  ...
  const setupRealTimeListener = useCallback((userId: string, characterId: string) => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (error) => {
        console.warn(`Real-time listener error for ${userId}/${characterId}:`, error);
      }
    );
  ...
  const resolveMultiple = useCallback(async (playerData: Array<{ userId: string; characterId: string }>) => {
    ...
    try {
      ...
      const characterResults = await Promise.all(
        Array.from(userGroups.entries()).map(async ([userId, characterIds]) => {
          const characterData = await fetchCharactersBatch(userId, characterIds);
          return { userId, characterData };
        })
      );
      
      // Create a lookup map for character data
      const characterLookup = new Map<string, Map<string, { name: string; avatar?: string }>>();
