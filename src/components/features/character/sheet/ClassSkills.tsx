...
export default function ClassSkills({
  ...
  const classSkills = useMemo(() => {
    ...
    if (!skillsForLevel) {
      logger.warn(
        `No skill data found for ${characterClassInfo.className} level ${level}`
      );
      return null;
    }
  ...
  const detailsContent = (
    <Card
      ...
      size="compact"
    >
      <Typography variant="h4" weight="semibold" className="mb-3 text-sm">
      ...
      <div className="mt-3 pt-3 border-t border-zinc-700">
        <div className="text-xs text-zinc-400">
          ...
          <List variant="disc" spacing="tight" size="sm">
            <ListItem>Skills improve automatically as you gain levels</ListItem>
            <ListItem>
              Success is based on rolling percentile dice (d100)
            </ListItem>
            <ListItem>Roll your skill percentage or lower to succeed</ListItem>
          </List>
        </div>
      </div>
    </Card>
      ...
      <div className={currentSize.container}>
        <div className="space-y-3">
          {Object.entries(ALL_SKILLS).map(([skillKey, skillLabel]) => {
            ...
            const trackingNote =
              (isRanger || isScout) && skillKey === "tracking"
                ? " (Wilderness only)"
                : "";
            const urbanPenalty =
              isRanger && (skillKey === "moveSilently" || skillKey === "hide")
