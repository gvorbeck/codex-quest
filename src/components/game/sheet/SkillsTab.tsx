...
import { CLASSES_WITH_SKILLS, SKILL_DESCRIPTIONS, ALL_SKILLS } from "@/constants/skills";
import type { Game } from "@/types/game";
import { usePlayerCharacters } from "@/hooks/usePlayerCharacters";
import { useSkillDataByClass, type ClassSkillData, type SkillTableRow } from "@/hooks/useSkillDataByClass";
import { useSkillColumns } from "@/hooks/useSkillColumns";
 ...
 */
const EnhancedSkillTable = memo(({ columns, data, caption }: EnhancedSkillTableProps) => {
  return (
          ...
          <tr role="row">
            {columns.map((column) => (
              <th
                ...
                role="columnheader"
                scope="col"
                aria-label={column.ariaLabel || (typeof column.header === 'string' ? column.header : undefined)}
              >
                {typeof column.header === 'string' && column.key in ALL_SKILLS && SKILL_DESCRIPTIONS[column.key as keyof typeof SKILL_DESCRIPTIONS] ? (
                  <span 
                    className="cursor-help border-b border-dotted border-amber-400/50"
                    title={SKILL_DESCRIPTIONS[column.key as keyof typeof SKILL_DESCRIPTIONS]}
                  >
                    {column.header}
                  </span>
                ) : column.header}
            ...
            <tr
              ...
              className={`
                ...
                ${row.isPlayer 
                  ...
                  : index % 2 === 1 ? 'bg-zinc-800/30' : ''
                }
              `}
              aria-label={row.isPlayer 
 ...
 */
const SkillClassTableWrapper = memo(({ classData }: { classData: ClassSkillData }) => {
  ...
  return (
    <Card className="overflow-hidden">
      <div className="p-4 border-b border-zinc-700">
        <Typography variant="h4" weight="semibold" id={`${classData.classId}-heading`}>
      ...
      {classData.skills.some(skill => skill.isPlayer) && (
        <div className="px-4 pb-4">
          <div className="flex items-center gap-2 text-xs text-zinc-400" role="note">
            <div className="w-3 h-3 bg-amber-900/20 border border-amber-500/30 rounded" aria-hidden="true"></div>
...
export const SkillsTab = memo(({ game, className }: SkillsTabProps) => {
      ...
      <Card className={className}>
        <div className="p-6 text-center">
          ...
          <Typography variant="caption" color="muted" className="mt-2 block">
            Unable to load player character data. Please try refreshing the page.
          </Typography>
        </div>
      </Card>
    );
    ...
    return (
      <Card className={className}>
        <div className="p-6 text-center">
          ...
          <Typography variant="caption" color="muted" className="mt-2 block">
            Classes with skills: {Object.values(CLASSES_WITH_SKILLS).map(c => c.displayName).join(", ")}
          </Typography>
        </div>
      </Card>
    );
  ...
  return (
    <div className={className} role="region" aria-label="Skill progression tables">
