...
export default function TreasureGeneratorModal({ 
  ...
  const [individualType, setIndividualType] = useState<IndividualTreasureType>("P");
  const [unguardedLevel, setUnguardedLevel] = useState<UnguardedTreasureLevel>(1);
  const [generatedTreasure, setGeneratedTreasure] = useState<TreasureResult | null>(null);
  ...
  const treasureConfig = useMemo(() => ({
    type: treasureType,
    subtype: treasureType === "lair" ? lairType 
      : treasureType === "individual" ? individualType 
      : unguardedLevel
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      const treasure = generateTreasure(treasureConfig.type, treasureConfig.subtype);
      ...
      const errorMessage = error instanceof Error ? error.message : "An unexpected error occurred while generating treasure";
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        showError("Failed to copy treasure to clipboard. Please select and copy manually.", {
          title: "Copy Failed",
          duration: 5000
        });
      }
    ...
    >
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3" role="radiogroup" aria-label="Treasure type selection">
            {TREASURE_TYPES.map((type) => (
              ...
              >
                <Icon name={type.icon as IconName} size="sm" aria-hidden={true} />
        ...
        <div className="space-y-3">
          ...
          {treasureType === "lair" && (
            <Select
              ...
              options={LAIR_TREASURE_TYPES.map(t => ({ value: t.value, label: t.label }))}
          ...
          {treasureType === "individual" && (
            <Select
              ...
              onValueChange={(value) => setIndividualType(value as IndividualTreasureType)}
              options={INDIVIDUAL_TREASURE_TYPES.map(t => ({ value: t.value, label: t.label }))}
          ...
          {treasureType === "unguarded" && (
            <Select
              ...
              onValueChange={(value) => setUnguardedLevel(parseInt(value) as UnguardedTreasureLevel)}
              options={UNGUARDED_TREASURE_LEVELS.map(t => ({ value: t.value.toString(), label: t.label }))}
        ...
        {error && (
          <div className="bg-red-50 dark:bg-red-900/20 border-2 border-red-300 dark:border-red-700 rounded-lg p-4">
            <div className="flex items-center gap-2">
              <Icon name="exclamation-triangle" size="sm" className="text-red-600 dark:text-red-400 flex-shrink-0" />
              <Typography variant="bodySmall" weight="semibold" className="text-red-800 dark:text-red-200">
                Error generating treasure
              </Typography>
            </div>
            <Typography variant="bodySmall" className="text-red-700 dark:text-red-300 mt-1 ml-6">
              {error}
            </Typography>
          </div>
        )}
            ...
            fallback={
              <Card className="bg-red-50 dark:bg-red-900/20 border-2 border-red-300 dark:border-red-700">
                <div className="p-4 text-center">
                  <Icon name="exclamation-triangle" size="md" className="text-red-600 dark:text-red-400 mx-auto mb-2" />
                  <Typography variant="h6" color="secondary" weight="semibold" className="mb-2 text-red-800 dark:text-red-200">
                    Error Displaying Treasure
                  </Typography>
                  <Typography variant="bodySmall" className="text-red-700 dark:text-red-300 mb-3">
                    There was a problem displaying the generated treasure. The treasure data may be corrupted.
                  </Typography>
                  <Button
                    variant="secondary"
                    size="sm"
            ...
            <div className="space-y-4">
              ...
              {shouldShowCoins && (
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-2 text-sm" role="list" aria-label="Coin amounts">
                    {coinsToDisplay.map((coin) => (
                      <div key={coin.key} className="flex items-center gap-2 p-2 bg-amber-50 dark:bg-amber-900/20 rounded border border-amber-200 dark:border-amber-700" role="listitem">
                        <Icon name="coin" size="xs" className={`${coin.color} dark:brightness-125`} aria-hidden={true} />
                        <span className="font-medium text-amber-900 dark:text-amber-100" aria-label={`${coin.amount} ${coin.key} pieces`}>
                ...
                <div>
                  ...
                  <div className={`grid gap-2 text-sm ${
                    ...
                    {generatedTreasure.gemsAndJewelry.map((item, index) => (
                      <div key={index} className="flex items-start gap-2 p-2 bg-purple-50 dark:bg-purple-900/20 rounded border border-purple-200 dark:border-purple-700">
                        <Icon name="star" size="xs" className="text-purple-600 dark:text-purple-400 mt-0.5 flex-shrink-0" />
                        <span className="text-purple-900 dark:text-purple-100 leading-tight">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className={`grid gap-2 text-sm ${
                    ...
                    {generatedTreasure.magicItems.map((item, index) => (
                      <div key={index} className="flex items-start gap-2 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border border-blue-200 dark:border-blue-700">
                        <Icon name="lightning" size="xs" className="text-blue-600 dark:text-blue-400 mt-0.5 flex-shrink-0" />
                        <span className="text-blue-900 dark:text-blue-100 leading-tight font-medium">{item}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="mt-3 p-3 bg-zinc-50 dark:bg-zinc-800/50 rounded border border-zinc-200 dark:border-zinc-700">
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-1 text-xs">
                      {generatedTreasure.breakdown.map((detail, index) => (
                        <li key={index} className="text-zinc-700 dark:text-zinc-300 py-0.5">
                          <span className="inline-block w-1 h-1 bg-zinc-400 dark:bg-zinc-500 rounded-full mr-2"></span>
                          {detail}
                        </li>
                      ))}
                    </ul>
